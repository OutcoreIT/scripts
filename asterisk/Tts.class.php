...
class Tts extends FreePBX_Helpers implements BMO {
	...
	/**
	 * Aqui é onde geramos o dialplan. Alteramos a linha do ext_agi
	 * para usar um arquivo temporário em vez de $ttstext direto.
	 */
	public function doDialplanHook(&$ext, $engine, $priority){
		...
		if (is_array($tts_list = $this->listTTS())) {
			foreach ($tts_list as $item) {
				...
				// Decodifica HTML antes de salvar em arquivo
				$decodedText = html_entity_decode($ttstext, ENT_QUOTES | ENT_HTML5, 'UTF-8');

				// Cria arquivo temporário contendo o texto completo
				$hash = md5($decodedText);
				$tmpfile = '/var/lib/asterisk/sounds/tts/tts-'.$hash.'.txt';
				file_put_contents($tmpfile, $decodedText);
				...
				// Chama AGI passando o caminho do arquivo .txt (em vez do texto)
				$ext->add($contextname, $ttsid, '',
					new \ext_agi(
						'elevenlabs-tts.agi,"'.$tmpfile.'",'.$ttsengine.','.$ttspath['path']
					)
				);
